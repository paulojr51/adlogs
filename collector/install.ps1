...
# Forca TLS 1.2 para todos os downloads (necessario em Windows Server antigo)
...
function Enable-WindowsAudit {
    ...
    foreach ($p in $policies) {
        $result = auditpol /set /subcategory:"$($p.sub)" /success:$($p.s) /failure:$($p.f) 2>&1
        if ($LASTEXITCODE -eq 0) {
            Write-Host "      [OK] $($p.sub)" -ForegroundColor Green
        } else {
            Write-Host "      [--] $($p.sub) (subcategoria nao encontrada, ignorando)" -ForegroundColor DarkGray
        }
    }

    # Garantir que auditoria por subcategoria esta habilitada (GPO pode bloquear)
    ...
    Write-Host ""
    Write-Host "Politicas aplicadas. Verificando:" -ForegroundColor Cyan
    auditpol /get /subcategory:"Logon","Logoff","Account Lockout"
}

# ─── Modo: apenas auditoria ───────────────────────────────────────────────────
if ($EnableAuditOnly) {
...
# ── PASSO 1: Python ────────────────────────────────────────────────────────────
Write-Step 1 $TOTAL_STEPS "Verificando Python 3.10+..."
...
try {
    ...
    if ($ver -match "Python 3\.(\d+)" -and [int]$Matches[1] -ge 10) {
        ...
        Write-Warn "Versao incompativel: $ver (necessario 3.10+)"
    }
} catch {
...
if (-not $pythonOk) {
    ...
    try {
        ...
        Write-Fail "Falha ao baixar Python: $_"
        Write-Fail "Baixe manualmente em https://www.python.org e instale com 'Add to PATH'."
        exit 1
    }
    ...
    try {
        ...
        Write-Fail "Python instalado mas nao encontrado no PATH. Feche e reabra o PowerShell como Admin e rode novamente."
        exit 1
    }
}

# Registrar pywin32 no Python do sistema (necessario para o servico Windows)
Write-Host "      Instalando/registrando pywin32 no sistema..." -ForegroundColor Yellow
pip install pywin32==308 --quiet 2>&1 | Out-Null
$postInstall = (python -c "import site; print(site.getsitepackages()[0])" 2>&1).Trim() + "\..\..\Scripts\pywin32_postinstall.py"
$postInstall = [System.IO.Path]::GetFullPath($postInstall)
if (Test-Path $postInstall) {
    python $postInstall -install -silent 2>&1 | Out-Null
    Write-OK "pywin32 registrado"
} else {
    # Fallback: tentar via caminho padrao
    $fallback = "C:\Program Files\Python311\Scripts\pywin32_postinstall.py"
    if (Test-Path $fallback) {
        python $fallback -install -silent 2>&1 | Out-Null
        Write-OK "pywin32 registrado"
    } else {
        Write-Warn "pywin32_postinstall.py nao localizado — servico pode falhar ao iniciar"
    }
}

# ── PASSO 2: Dependencias no sistema ──────────────────────────────────────────
Write-Step 2 $TOTAL_STEPS "Instalando dependencias Python no sistema..."
...
# ── PASSO 3: Ambiente virtual (para desenvolvimento/debug) ────────────────────
Write-Step 3 $TOTAL_STEPS "Criando ambiente virtual (debug)..."
...
Write-OK "Venv pronto em $VenvPath"

# ── PASSO 4: Configurar .env ──────────────────────────────────────────────────
Write-Step 4 $TOTAL_STEPS "Configurando .env..."
...
if ($needsConfig) {
    ...
    $apiUrl = Read-Host "  URL da API (Enter para usar $defaultUrl)"
    if ([string]::IsNullOrWhiteSpace($apiUrl)) { $apiUrl = $defaultUrl }

    # API Key
    do {
        $apiKey = Read-Host "  Chave de acesso do servidor (SERVER_API_KEY)"
        $apiKey = $apiKey.Trim()
    ...
    if ([string]::IsNullOrWhiteSpace($intervalStr) -or -not ($intervalStr -match '^\d+$')) {
        $interval = 30
    } else {
...
SERVICE_DESCRIPTION=Coleta eventos de login e acesso a arquivos do Windows Event Log e persiste no PostgreSQL.
"@
...
Write-Step 5 $TOTAL_STEPS "Instalando servico Windows..."

Set-Location $CollectorDir

# Remover versao anterior se existir
...
# Configurar reinicio automatico em caso de falha
...
if ($svc -and $svc.Status -eq "Running") {
    Write-OK "Servico iniciado e rodando"
} else {
    Write-Fail "Servico nao iniciou. Verifique o log: C:\ProgramData\ADLogs\collector.log"
    Write-Host "      Tentando iniciar manualmente..." -ForegroundColor Yellow
    Start-Service $ServiceName -ErrorAction SilentlyContinue
    Start-Sleep -Seconds 5
    $svc = Get-Service -Name $ServiceName -ErrorAction SilentlyContinue
    if ($svc -and $svc.Status -eq "Running") {
        Write-OK "Servico iniciado na segunda tentativa"
    } else {
        Write-Fail "Falha ao iniciar. Execute: .\venv\Scripts\python.exe service.py debug"
    }
}

# ── PASSO 6: Auditoria Windows ────────────────────────────────────────────────
Write-Step 6 $TOTAL_STEPS "Habilitando auditoria do Windows..."
Enable-WindowsAudit
